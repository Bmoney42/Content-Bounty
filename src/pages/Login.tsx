...
const Login: React.FC = () => {
  ...
  const { login, loading, isAuthenticated, rateLimitInfo } = useAuth()
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 text-lg">
            Sign in to your Creator Bounty account
          </p>
        </div>

        {/* Rate Limit Warning */}
        <RateLimitWarning
          isBlocked={rateLimitInfo.loginBlocked}
          remainingAttempts={rateLimitInfo.loginAttemptsRemaining}
          actionType="login"
          className="mb-6"
        />
        ...
        <div className="bg-white/80 backdrop-blur-sm border border-white/20 rounded-2xl p-8 shadow-xl">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={loading || rateLimitInfo.loginBlocked}
            ...
            >
              {loading ? 'Signing in...' : rateLimitInfo.loginBlocked ? 'Account Locked' : 'Sign in'}
            </button>
          </form>
          ...
          {import.meta.env.DEV && (
            <div className="mt-8 pt-6 border-t border-gray-200">
              <div className="text-center mb-4">
                <div className="inline-flex items-center space-x-2 px-4 py-2 bg-yellow-50 text-yellow-700 rounded-full">
                  <CheckCircle className="w-4 h-4" />
                  <span className="text-sm font-semibold">Development Demo</span>
                </div>
              </div>
              
              <div className="bg-gradient-to-r from-gray-50 to-gray-100 border border-gray-200 rounded-xl p-4">
                <div className="text-xs text-gray-600 font-semibold mb-1">Demo Account (Dev Only)</div>
                <div className="text-sm text-gray-900 font-mono">demo@example.com / demo123</div>
                <div className="text-xs text-gray-500 mt-2">⚠️ Demo mode with placeholder data</div>
              </div>
            </div>
          )}
